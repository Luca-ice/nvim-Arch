return {
  ...
  config = function()
    require('bufferline').setup {
      options = {
        mode = 'buffers', -- Set to "tabs" for Neovim's tabpages, "buffers" for open files
        -- `nvim_set_option` is no longer needed in newer versions of bufferline
        separator_style = 'slant', -- "slant", "padded_slant", "thin", or a table like { '>', '<' }
        always_show_bufferline = true,
        show_buffer_close_icons = true,
        show_close_icon = true,
        diagnostics = 'nvim_lsp', -- Show LSP diagnostics (errors/warnings)
        diagnostics_indicator = function(_, _, diagnostics_info)
          local s = ' '
          for _, diagnostic_count in pairs(diagnostics_info) do
            s = s .. diagnostic_count.count .. diagnostic_count.icon .. ' '
          end
          return s
        end,
        offsets = {
          {
            filetype = 'neo-tree', -- Adjust if your file explorer has a different filetype
            text = 'File Explorer', -- Text to display in the offset
            text_align = 'left',
            separator = true,
          },
        },
        sort_by = 'insert_after_current', -- "id", "extension", "relative_directory", etc.
        -- You can also specify custom sections, colors, etc.
      },
      -- Add custom themes or highlights here if needed
      highlights = {
        -- Example: Customize background for active buffer
        buffer_selected = {
          fg = { attribute = 'fg', highlight = 'Normal' },
          bg = { attribute = 'bg', highlight = 'Normal' },
          bold = true,
        },
        -- You can also customize groups like BufferClose, BufferModified, etc.
      },
    }

    -- Set up keybindings for bufferline
    -- These are common binds similar to LazyVim or other popular configs
